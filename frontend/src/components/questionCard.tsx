import { Trash, SquarePen, Terminal, TextAlignStart, SquareCheckBig } from 'lucide-react';
    ...
    <div className="flex flex-col gap-3 shadow-sm hover:shadow-md bg-white border rounded-lg p-4 group transition-colors">
      <div className="flex justify-between">
        ...
        <div className="flex opacity-0 group-hover:opacity-100 duration-200">
          <Button className="bg-transparent hover:bg-slate-100 text-slate-600" onClick={()=> navigate(`/recruiter/question-bank/update/${question.id}`)}><SquarePen/></Button>
          <Button className="bg-transparent hover:bg-slate-100 text-slate-600" onClick={()=> onDelete(question.id)}><Trash/></Button>
        </div>
      </div>
